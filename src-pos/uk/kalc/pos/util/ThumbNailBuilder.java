...
public class ThumbNailBuilder {
    ...
    public ThumbNailBuilder(int width, int height, ImageIcon img) {
        m_width = width;
        m_height = height;

        Image defimg = img.getImage();
        if (defimg == null) {
            m_imgdefault = null;
        } else {
            m_imgdefault = createThumbNail(defimg);
        }
    }
    ...
    public ImageIcon getThumbNailIcon(Image img) {
        if (img == null) {
            return new ImageIcon(m_imgdefault);
        } else {
            return new ImageIcon(createThumbNail(img));
        }
    }

    private Color getContrastColour() {
        Color color = new Color((int) Integer.decode(SystemProperty.BUTTONTEXTCOLOUR));
        double luminance = 0.2126 * color.getRed() + 0.7152 * color.getGreen() + 0.0722 * color.getBlue();

        // choose the contrasting color based on the luminance
        if (luminance > 128) {
    ...
    public Image getThumbNailText(Image img, String text) {
        ...
        label.setOpaque(!SystemProperty.TEXTOPAQUE);
        label.setText(text);
        label.setForeground(new Color((int) Integer.decode(SystemProperty.BUTTONTEXTCOLOUR)));
        label.setBackground(getContrastColour());
        ...
        // The background
        Color c1 = new Color(0xff, 0xff, 0xff, 0x0);
        Color c2 = new Color(0xff, 0xff, 0xff, 0x00);
    ...
    public Image getThumbNailText(Image img, String text, Color colour) {
